...
class UserCardAPP {
	...
	public function load($uids) {
		...
		foreach ($arr as $uid){
			if(is_numeric($uid)){
				...
				$uid > 0 || $uid = 'guest';
				$html .= '<font class="ucard" uid="' . $uid . '">' . $uName . '</font>&nbsp;&nbsp;';
			}
		}
		return $html;
	}
	...
	public function recharge() {
		...
		$r = logic ( 'me' )->money ()->add ( $money, $uid, array (
				'name' => '管理员后台充值',
				'intro' => $remark 
		) );
	...
	public function lessmoney() {
		...
		$money = get ( 'money', 'float' );
		$remark = get ( 'remark', 'txt' );
		$r = logic ( 'me' )->money ()->less ( $money, $uid, array (
				'name' => '管理员后台扣费',
				'intro' => $remark 
		) );
	...
	public function send_mail() {
		...
		$title = get ( 'title', 'txt' );
		if (trim ( $title ) == '')
			$this->ajax_response ( 'err', '请输入邮件标题！' );
		$content = get ( 'content' );
		if (trim ( $content ) == '')
			$this->ajax_response ( 'err', '请输入邮件内容！' );
		logic ( 'push' )->addi ( 'mail', user ( $uid )->get ( 'email' ), array (
				'subject' => $title,
				'content' => $content 
		) );
	...
	public function send_sms() {
		...
		if (trim ( $content ) == '')
			$this->ajax_response ( 'err', '请输入短信内容！' );
		logic ( 'push' )->addi ( 'sms', user ( $uid )->get ( 'phone' ), array (
				'content' => $content 
		) );
