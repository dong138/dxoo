...
(function(style){
	if(!style)
	{
		...
		link.href = _path + '../images/skins/' + _skin + '.css';
	    link.rel = 'stylesheet';
...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	_init: function( config )
	{
		...
		if( config.icon && !isIfr )
		{
			...
			DOM.icon[0].innerHTML = '<img src="' + config.path + '../images/skins/icons/' + config.icon + '" class="ui_icon_bg"/>';
		}
		else
	...
	_ie6PngFix: function()
	{
	    if( _ie6 )
		{
			var i = 0, elem, png, pngPath, runtimeStyle,
				path = lhgdialog.setting.path + '../images/skins/',
...
lhgdialog.tips = function( content, time, icon, callback )
{
	var reIcon = icon ? function(){
		this.DOM.icon[0].innerHTML = '<img src="' + this.config.path + '../images/skins/icons/' + icon + '" class="ui_icon_bg"/>';
