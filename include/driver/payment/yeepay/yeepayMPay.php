...
class yeepayMPay {
	...
	private $API_Mobile_Pay_Base_Url = 'https://ok.yeepay.com/paymobile/mobile/';
	private $API_PC_Pay_Base_Url = 'https://ok.yeepay.com/payweb/';
 	private $API_Merchant_Base_Url = 'https://ok.yeepay.com/merchant/';

	public function __construct($payment){
		$this->account = $payment['config']['merchantaccount'];
		$this->merchantPublicKey = $payment['config']['merchantPublicKey'];
		$this->merchantPrivateKey = $payment['config']['merchantPrivateKey'];
		$this->yeepayPublicKey = $payment['config']['yeepayPublicKey'];
		...
		$query = array(
			...
			'callbackurl'	=>	$data['backurl'],
			'fcallbackurl'	=>	$data['backurl']
		);
		return $this->getUrl(YEEPAY_PC_API, 'api/pay/request', $query, 'post');
	}

		public function creditWebPay($data){
		$query = array(
			'orderid'	=>	$data['orderid'],
			'transtime'	=>	time(),
			'currency'	=>	156,
			...
			'userua'	=>	$_SERVER['HTTP_USER_AGENT'],
			'callbackurl'	=>	$data['backurl'],
			'fcallbackurl'	=>	$data['fbackurl']
		);
		return $this->getUrl(YEEPAY_MOBILE_API, 'pay/bankcard/credit/request', $query, 'get');
	}

		public function debitWebPay($data){
		$query = array(
			'orderid'	=>	$data['orderid'],
			'transtime'	=>	time(),
			'currency'	=>	156,
		...
		return $this->getUrl(YEEPAY_MOBILE_API, 'pay/bankcard/debit/request', $query, 'get');
	}
	...
	protected function getUrl($type,$method,$query,$pg){
		$request = $this->buildRequest($query);
		$url = $this->getAPIUrl($type,$method);
		if($pg == 'get'){
			$url .= '?'.http_build_query($request);
