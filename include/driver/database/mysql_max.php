...
class mysql_maxDatabaseDriver {
			...
			if (is_array ( $data )) {
				foreach ( $data as $key => $val ) {
					...
					if (is_numeric ( $val )) {
						...
						$sql_data_add .= '`' . $key . '`' . '="' . ($dbo->pack_data_escape ? mysql_escape_string ( $val ) : $val) . '"';
					} else {
						$noData = true;
					}
					$noData || $sql_data_add .= ', ';
	...
	private function trace($message) {
		...
		$this->_trace [] = array (
				...
				'mmusage' => function_exists ( 'memory_get_usage' ) ? memory_get_usage () : 0,
				'message' => $message 
		);
	}
