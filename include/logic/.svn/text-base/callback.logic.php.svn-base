...
class Order_Parser_CallbackLogic extends CallbackLogic {
	...
	function Parse_WAIT_SELLER_SEND_GOODS($payment = false) {
		...
		if (! $error) {
			if ($order ['product'] ['type'] == 'ticket') {
				...
				$isCOD = $trade ['nmpay'];
				$isCOD && logic ( 'order' )->Update ( $trade ['sign'], array (
						'pay' => ORD_PAID_Yes,
						'process' => '__PAY_YET__' 
				) );
				logic ( 'order' )->MakeSuccessed ( $trade ['sign'] );
				$isCOD && logic ( 'order' )->Update ( $trade ['sign'], array (
						'pay' => ORD_PAID_No,
						'process' => 'WAIT_SELLER_SEND_GOODS' 
				) );
			}
		} else {
			if ($order ['_tmpd_'] ['paid']) {
				logic ( 'order' )->Update ( $trade ['sign'], array (
		...
		if (logic ( 'pay' )->Process ( $payment, 'WAIT_SELLER_SEND_GOODS' ))
			return;
	...
	function Parse_WAIT_BUYER_CONFIRM_GOODS($payment = false) {
		...
		if (logic ( 'pay' )->Process ( $payment, 'WAIT_BUYER_CONFIRM_GOODS' ))
			return;
	...
	function Parse_TRADE_FINISHED($payment = false) {
		...
		if (logic ( 'pay' )->Process ( $payment, 'TRADE_FINISHED' ))
			return;
