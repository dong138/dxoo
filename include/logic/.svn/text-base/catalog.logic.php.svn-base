...
class CatalogLogic {
		...
		foreach ( $class as $i => $topclass ) {
			...
			if ($topclass ['id'] > 0) {
				...
				if ($subclass) {
					foreach ( $subclass as $n => $value ) {
						if ($value ['flag']) {
							$subclass [$n] ['url'] = logic ( 'url' )->create ( 'seller', array (
									'catalog' => $value ['flag'] 
							) );
						} else {
							$subclass [$n] ['url'] = logic ( 'url' )->create ( 'seller', array (
									'catalog' => $topclass ['flag'] 
							) );
						}
						$subclass [$n] ['selected'] = $value ['flag'] == $subcss ? true : false;
					}
			...
			$class [$i] ['selected'] = $topclass ['flag'] == $topcss ? true : false;
			$class [$i] ['url'] = logic ( 'url' )->create ( 'seller', array (
					'catalog' => $topclass ['flag'] 
			) );
		}
		$this->ProCountSync ();
	...
	public function Flag2Name($flag) {
		$infos = dbc ( DBCMax )->select ( 'catalog' )->in ( 'name' )->where ( array (
				'flag' => $flag 
		) )->limit ( 1 )->done ();
	...
	public function GetList($parent = 0, $sortstr = 0) {
		...
		if ($sortstr == 2 && $parent == 0) {
			//控制分类导航的数目
			//TODO
			$return = $catalogOBJ->limit ( 10 )->done ();
	...
	private function Filter_topClass($classFlag, $prefix = '') {
		$topClass = $this->Search ( array (
				'flag' => $classFlag 
		) );
		$this->urlTopClass = $topClassID = $topClass ['id'];
		$subClasses = ( array ) $this->Search ( array (
				'parent' => $topClassID 
		), 0 );
