...
class CityManageLogic {
	...
	public function place_inputer($product_id) {
		...
		if ($product_id || MEMBER_ROLE_TYPE == 'seller') {
			if ($product_id) {
				$product = dbc ( DBCMax )->select ( 'product' )->where ( array (
						'id' => $product_id 
				) )->limit ( 1 )->done ();
			...
			$places = $this->make_selected ( $places, array (
					'region:' . $product ['city_place_region'],
					'street:' . $product ['city_place_street'] 
			) );
		}
	...
	public function seller_place_inputer($seller_id) {
		...
		if ($seller_id || MEMBER_ROLE_TYPE == 'seller' || 1 == MEMBER_ID) {
			if ($seller_id) {
				$seller = dbc ( DBCMax )->select ( 'seller' )->where ( array (
						'id' => $seller_id 
				) )->limit ( 1 )->done ();
			} elseif (MEMBER_ROLE_TYPE == 'seller') {
			...
			$places = $this->make_selected ( $places, array (
					'region:' . $seller ['city_place_region'],
					'street:' . $seller ['city_place_street'] 
			) );
		}
		include handler ( 'template' )->file ( '@admin/city_place_inputer' );
	}
	...
	private function make_links($places, $flag = 'product') {
		foreach ( $places as $i => $region ) {
			$places [$i] ['url'] = logic ( 'url' )->create ( $flag, array (
					'region' => $region ['id'],
					'street' => null 
			) );
			if (isset ( $region ['streets'] )) {
				foreach ( $region ['streets'] as $ii => $street ) {
					$places [$i] ['streets'] [$ii] ['url'] = logic ( 'url' )->create ( $flag, array (
							'street' => $street ['id'] 
					) );
				}
			}
	...
	private function make_selected($places, $filter) {
		...
		if (count ( $region_sels ) > 1) {
			asort ( $region_sels );
			array_pop ( $region_sels );
			foreach ( $region_sels as $id => $ix ) {
				if ($ix)
					unset ( $places [$id] ['selected'] );
			}
		}
		return $places;
	}
	...
	public function get_place_one($id) {
		return dbc ( DBCMax )->select ( 'city_place' )->where ( array (
				'id' => $id 
		) )->limit ( 1 )->done ();
	}
	...
	public function del_place($id) {
		...
		if ($old) {
			if ($old ['type'] != 'street') {
				dbc ( DBCMax )->delete ( 'city_place' )->where ( array (
						'parent_type' => $old ['type'],
						'parent_id' => $old ['id'] 
				) )->done ();
			}
			$this->product_place_clear ( $old ['type'], $id );
			return dbc ( DBCMax )->delete ( 'city_place' )->where ( array (
					'id' => $id 
			) )->done ();
		} else {
			return false;
	...
	public function get_of_parent($type, $id) {
		$datas = dbc ( DBCMax )->select ( 'city_place' )->where ( array (
				'parent_type' => $type,
				'parent_id' => $id 
		) )->done ();
		$datas || $datas = array ();
	...
	private function product_place_clear($type, $id) {
		...
		dbc ( DBCMax )->update ( 'product' )->data ( $data )->where ( array (
				$field => $id 
		) )->done ();
	}
    ...
    {
        $data = dbc ( DBCMax )->select ( 'city' )->done();
        return $data;
    }
    public function getCity($type ='city',$id)
    {
       return $this->get_of_parent ( $type, $id );
    }
}
