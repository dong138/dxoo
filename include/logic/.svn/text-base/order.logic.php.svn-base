...
class OrderLogic {
	...
	public function GetList($uid = 0, $status = ORD_STA_ANY, $coupon_status, $pay = ORD_PAID_ANY, $extend = '1', $notpage = 1) {
		$sql_limit_user = '1';
		if ($uid > 0) {
		...
		$sql_limit_coupon = '1';
		if ($coupon_status >= -1) {
			$sql_limit_coupon = 't.status = ' . $coupon_status;
		}
		if ($status == ORD_ZP_FAIL) {
			$sql_limit_status = 'status != ' . ORD_ZP_FAIL;
		}
		$sql_limit_pay = '1';
		if ($pay >= 0) {
			$sql_limit_pay = 'pay = ' . $pay . ' AND `process`<>"_TimeLimit_" ';
		}
		$sql = 'SELECT o.*, m.username, t.status coupon_status , t.usetime  
		FROM
			' . table ( 'order' ) . ' o 
		LEFT JOIN 
			' . table ( 'members' ) . ' m 
	...
	public function FreeCountDownOrder($productid = 0) {
		...
		$timelimit = time () - (60 * $timeLimit_free);
		...
		if (count ( $data ) > 0) {
			foreach ( $data as $v ) {
				dbc ( DBCMax )->update ( 'order' )->data ( 'process="_TimeLimit_"' )->where ( 'orderid=' . $v ['orderid'] )->done ();
				dbc ( DBCMax )->update ( 'product' )->data ( 'sells_count=sells_count-' . $v ['productnum'] )->where ( 'id=' . $v ['productid'] )->done ();
			}
		}
	}
	/**
	 * 20150203订单过期处理和特权产品过期处理
	 * @param number $productid
	 */
	public function FreeCountDownOrderNew($productid = 0) {
		//关联查询所有限购的订单
		$sql_where_process = ($productid > 0 ? " AND a.productid='" . $productid . "'" : '') . ' AND a.process<>"_TimeLimit_" AND (a.process="__CREATE__" OR a.process="WAIT_BUYER_PAY")';
		$sql = "select a.*,b.order_time as order_time from ystttuangou_order a left join ystttuangou_product b on a.productid = b.id where a.is_countdown = 1".$sql_where_process;
		$orders = dbc ()->Query ( $sql )->GetAll ();
		//循环订单，判断时间，设置过期，设置购买数量
		if(count ( $orders ) > 0){
			foreach ( $orders as $v ) {
				//计算时间
				$timelimit = 0;
				if($v['order_time']){
					//设置了时间
					$timelimit = time () - (60 * $v['order_time']);
				}else{
					//没有设置时间
					$timeLimit_free = ini ( 'settings.free_time' );
					$timelimit = time () - (60 * $timeLimit_free);
				...
				//判断时间
				if($v['buytime'] < $timelimit){
					//设置订单过期
					dbc ( DBCMax )->update ( 'order' )->data ( 'process="_TimeLimit_"' )->where ( "orderid='" . $v ['orderid']."'" )->done ();
					//设置特权产品过期
					dbc ( DBCMax )->update ( 'activity_special' )->data ( 'status=2' )->where ( "orderid='" . $v ['orderid']."'" )->done ();
					//设置购买数量
					dbc ( DBCMax )->update ( 'product' )->data ( 'sells_count=sells_count-' . $v ['productnum'] )->where ( 'id=' . $v ['productid'] )->done ();
				}
			}
			...
			if (! $trade ['nmpay']) {
				...
				if ($error) {
					...
					if ($order ['_tmpd_'] ['paid'] === false) {
						...
						if (in_array ( $order ['process'], array (
								'__CREATE__',
								'WAIT_BUYER_PAY' 
						) )) {
	...
	public function MakeSuccessed($orderid, $ignore_rules = false) {
		...
		if ($order ['product'] ['type'] == 'ticket') {
			...
			logic ( 'order' )->clog ( $orderid )->sys ( TUANGOU_STR . '券已经生成：交易完成' );
		} else {
		...
		$notify_type = $order['openid'] ? 1 : 0; //20150424 by cqx
		$this->notifyWechat($order['orderid'],$notify_type,'order');
		//是否需要发送短信
		$myproduct = logic('product')->SrcOne($order ['productid']);
		if($myproduct['is_sms']){
	 ...
	 * $type 通知类型 (方法名称)
     */
	public function notifyWechat($commonid=0,$notify_type=1,$type=''){
		//$url = ini('settings.iweb_site_url') . "/weixin/index.php?s=/home/YsIndex/sendTicket/commonid/$commonid/wechat/$notify_type/type/$type";
		$url = "http://192.168.22.3/weixin/index.php?s=/home/YsIndex/sendTicket/commonid/$commonid/wechat/$notify_type/type/$type";
		//$url = "http://cqxlient.vicp.net/weixin/index.php?s=/home/YsIndex/sendTicket/commonid/$commonid/wechat/$notify_type/type/$type";
	...
	private function __parse_result($data) {
		...
		$data ['paymentinfo'] = $data ['paytype'] > 0 ? logic ( 'pay' )->GetOne ( $data ['paytype'] ) : array (
				'name' => '-----' 
		);
		return $data;
	}
	...
	public function Refund($oid, $inRFM = null) {
		...
		logic ( 'order' )->Update ( $oid, array (
				'status' => ORD_STA_Refund 
		) );
		logic ( 'order' )->doFailed ( $order );
		if ($order ['product'] ['type'] == 'ticket') {
			dbc ( DBCMax )->update ( 'ticket' )->where ( 'orderid=' . $order ['orderid'] . '	AND status=' . TICK_STA_Unused )->data ( 'status=' . TICK_STA_Invalid )->done ();
		}
	...
	public function doFailed($order) {
		...
		logic ( 'seller' )->money_less ( $sid, ($order ['totalprice'] - $order ['expressprice']) );
	...
	public function getCouponStatus($id){
		$sql = 'SELECT t.status coupon_status FROM '. TABLE_PREFIX .'tttuangou_ticket t WHERE orderid = '.$id;
		return $coupon = dbc ( DBCMax )->query ( $sql )->limit(1)->done ();
// 		return $data = dbc ( DBCMax )->select ( 'ticket.status' )->where ( 'orderid='.$id )->limit ( 1 )->done ();
	}
	
	public function GetProcessValue($pid) {
		$value = "";
		if ($pid == "__CREATE__") {
			$value = "创建订单";
			return $value;
		...
		if ($pid == "WAIT_BUYER_PAY") {
			$value = "等待付款";
			return $value;
		}
		if ($pid == "WAIT_SELLER_SEND_GOODS") {
			$value = "等待发货";
			return $value;
		}
		if ($pid == "WAIT_BUYER_CONFIRM_GOODS") {
			$value = "等待收货";
			return $value;
		}
		if ($pid == "TRADE_FINISHED") {
			$value = "交易完成";
			return $value;
		}
		return $value;
...
class OrderLogic_cLog {
	...
	function vlist($action = '1') {
		return dbc ( DBCMax )->select ( 'order_clog' )->where ( 'sign=' . $this->CSIGN )->where ( $action )->order ( 'id.desc' )->done ();
	}
}
?>
