...
class MeLogic {
	...
	function ticketCreate($userid, $productid, $orderid) {
		...
		$ary = array (
				...
				'orderid' => $orderid,
				'number' => $ticketNumber,
				'password' => authcode ( $ticketPassword, 'ENCODE', $this->Config ['auth_key'] ),
				'status' => 1 
		);
		...
		if ($sms ['power'] == 'on') {
			...
			if (is_numeric ( $userInfo ['phone'] )) {
				...
				$smsContent = str_replace ( array (
						...
						$ticketNumber,
						$ticketPassword,
						date ( 'Y-m-d', $ticketInfo ['perioddate'] ),
						$ticketInfo ['sellerphone'],
						$ticketInfo ['selleraddress'],
	...
	function SendUseMail($id) {
		...
		$query = $this->DatabaseHandler->Query ( $sql );
		$ticket = $query->GetRow ();
		if ($ticket ['userid'] != MEMBER_ID || $ticket == '')
			return false;
		$ary = array (
				'address' => $ticket ['email'],
				'username' => $ticket ['username'],
				'title' => __ ( '购物券即将到期提示信息' ),
				'content' => '温馨提示您购买的产品（' . $ticket ['name'] . '）' . TUANGOU_STR . '券即将于' . date ( 'Y-m-d', $ticket ['perioddate'] ) . '到期请尽快消费以免过期，请点<a href="' . $this->Config ['site_url'] . '">这里</a>查看您的' . TUANGOU_STR . '券！',
				'addtime' => time () 
		);
		$keys = $values = '';
		foreach ( $ary as $i => $valuez ) {
...
// 		$result = $this->DatabaseHandler->Insert ( $ary );
		dbc ( DBCMax )->insert ( 'usermsg' )->data ( $ary )->done ();
		return true;
	}
...
class MeLogic_User {
	...
	public function field($key, $val = false, $life = 0, $readAll = false) {
		...
		if (is_null ( $val )) {
			return dbc ( DBCMax )->delete ( 'metas' )->where ( array (
					'uid' => $uid,
					'key' => $key 
			) )->done ();
		}
		if (! $val) {
			$row = dbc ( DBCMax )->select ( 'metas' )->where ( array (
					'uid' => $uid,
					'key' => $key 
			) )->limit ( 1 )->done ();
			if (! $row) {
		...
		if ($old) {
			$dbc = dbc ( DBCMax )->update ( 'metas' )->where ( array (
					'id' => $old ['id'] 
			) );
			$data = array (
					'val' => $val,
...
class MeLogic_Money {
	...
	public function less($moves, $uid = 0, $log = array()) {
		...
		if ($userinfo ['money'] > 0 && $moves > 0 && $moves <= $userinfo ['money']) {
			$sql = 'UPDATE ' . table ( 'members' ) . ' SET money = money - ' . $moves . ' WHERE uid = ' . $uid;
			$query = dbc ( DBCMax )->query ( $sql )->done ();
			$money = floatval ( logic ( 'me' )->money ()->count ( $uid ) );
