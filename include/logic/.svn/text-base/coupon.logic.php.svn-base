...
class CouponLogic {
	...
	public function TicketGet($number) {
		$coupon = dbc ( DBCMax )->select ( 'ticket' )->where ( array (
				'number' => $number 
		) )->limit ( 1 )->done ();
		$coupon || $coupon = array ();
		if ($coupon) {
			$product = dbc ( DBCMax )->select ( 'product' )->in ( 'sellerid' )->where ( array (
					'id' => $coupon ['productid'] 
			) )->limit ( 1 )->done ();
			if ($product) {
				$seller = dbc ( DBCMax )->select ( 'seller' )->in ( 'userid' )->where ( array (
						'id' => $product ['sellerid'] 
				) )->limit ( 1 )->done ();
				if ($seller) {
		...
		if ($coupon ['status'] == TICK_STA_Unused) {
			$other_coupons = dbc ( DBCMax )->select ( 'ticket' )->where ( array (
					'uid' => $coupon ['uid'],
				...
				case TICK_STA_Unused :
					if ($product ['sellerid'] != logic ( 'seller' )->U2SID ( $ops_member_id )) {
						...
						if ($product ['coupon'] ['password'] != $password) {
							...
							if ($morecoupon) {
								$morenumbers = explode ( ',', $morecoupon );
							...
							foreach ( $checknumbers as $k => $v ) {
								$allnumbers [] = $v ['number'];
								$product ['coupon'] = $v;
								dbc ( DBCMax )->update ( 'ticket' )->data ( array (
										'status' => TICK_STA_Used,
										'usetime' => $usetime 
								) )->where ( array (
							...
							if (count ( $checknumbers ) > 1) {
								return $this->mu_success ( $allnumbers );
							} else {
	 ...
	 * @param string $morecoupon
	 * @param unknown $sellerid
	 * @param unknown $clerkaid
	 * @return multitype:boolean unknown
	 */
	public function appMakeUsed($number, $password,$morecoupon='',$sellerid,$clerkaid) {
		$product = $this->ProductGet ( $number );
		if ($product) {
			switch ($product ['coupon'] ['status']) {
				case TICK_STA_Unused :
					if ($product ['coupon'] ['password'] != $password) {
						...
						foreach ( $checknumbers as $k => $v ) {
							...
							# 微信消息推送  $commonid 优惠券ID ,$notify_type=2 (默认) $type=useTicket 使用团购券
							logic ( 'order' )->notifyWechat($product ['coupon'] ['orderid'],2,'useTicket');
						...
						//验证成功
						if (count ( $checknumbers ) > 1) {
							return $this->appmu_success ( $allnumbers, $product ['name']);
						} else {
							return $this->appmu_success ( $product ['coupon'], $product ['name']);
						}
					}
					break;
				case TICK_STA_Used :
					return $this->appmu_error ( 'be-used', $product ['coupon'], $product ['name']);
					break;
				case TICK_STA_Overdue :
					return $this->appmu_error ( 'be-overdue', $product ['coupon'], $product ['name']);
					break;
				case TICK_STA_Invalid :
					return $this->appmu_error ( 'be-invalid', $product ['coupon'], $product ['name']);
					break;
			}
		} else {
			return $this->appmu_error ( 'not-found' );
		}
	}
	...
	private function appmu_error($errcode, $coupon = array(),$name) {
		return array (
				'error' => true,
				'errcode' => $errcode,
				'coupon' => $coupon,
	...
	private function appmu_success($coupon,$name) {
		return array (
				'error' => false,
				'coupon' => $coupon,
				'name' => $name
