...
class iImagerUI
{
	...
	public function adplay_single($id, $iids)
	{
		if(false == is_array($iids)) {
			$iids = explode(',', $iids);
		}
		$iid = $iids[0];
		include handler('template')->file('@html/iimager/'.$this->style.'/adplay_single');
	}
	public function adplay_multis($id, $iids)
	{
		if(false == is_array($iids)) {
			$iids = explode(',', $iids);
		}
		$iids = (array) $iids;
		include handler('template')->file('@html/iimager/'.$this->style.'/adplay_multis');
	} 
}
