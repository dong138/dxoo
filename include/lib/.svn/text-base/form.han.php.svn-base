...
class FormHandler {
	...
	function TimeSelect($name, $selected = '') {
		...
		$options = array (
				...
				array (
						"name" => __ ( "最近一天" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day - 1 ) 
				),
				array (
						"name" => __ ( "最近两天" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day - 2 ) 
				),
				array (
						"name" => __ ( "最近三天" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day - 3 ) 
				),
				array (
						"name" => __ ( "最近一周" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day - 7 ) 
				),
				array (
						"name" => __ ( "最近一个月" ),
						"value" => mktime ( 0, 0, 0, $this_month - 1, $this_day ) 
				),
				array (
						"name" => __ ( "最近两个月" ),
						"value" => mktime ( 0, 0, 0, $this_month - 2, $this_day ) 
				),
				array (
						"name" => __ ( "最近三个月" ),
						"value" => mktime ( 0, 0, 0, $this_month - 3, $this_day ) 
				),
				array (
						"name" => __ ( "最近半年" ),
						"value" => mktime ( 0, 0, 0, $this_month - 6, $this_day ) 
				),
				array (
						"name" => __ ( "最近一年" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day, $this_year - 1 ) 
				),
				array (
						"name" => __ ( "最近三年" ),
						"value" => mktime ( 0, 0, 0, $this_month, $this_day, $this_year - 3 ) 
				) 
		);
...
class FormWrap {
	...
	function AddElement($name, $element, $describe = '') {
		$this->Element [$this->_group] ['element'] [] = array (
				'name' => $name,
				'element' => $element,
				'describe' => $describe 
		);
	...
	function AddGroup($name, $extra = '') {
		$group = md5 ( $name );
		$this->_group = $group;
		$this->Element [$this->_group] = array (
				'name' => $name,
				'extra' => $extra 
		);
	}
