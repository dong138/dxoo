...
define ( "MAIL_TO", "kmwangxiao01@163.com" );
class ErrorHandler {
	var $_type;
	var $_message;
	var $_sql;
	...
	function fatal() {
		...
		$detail_list = array (
				"<h3>{$type_title} [$this->_type]: <em>{$this->_message}</em></h3>" 
		);
		...
		if (SEND_ERROR_MAIL && DEBUG == false) {
			...
			foreach ( $to_list as $to ) {
				send_mail ( $to, "({$config['site_name']}){$type_title} [$this->_type]", $this->Template ( $details ), $config ['site_name'], $config ['site_admin_email'] );
			}
		}
		if (DEBUG == false)
			$details = __ ( '您不是系统管理人员，不能查看错误信息。<br />相关错误信息已经发给系统管理人员，我们将尽快修复错误。' );
		...
		$errer_msg = "<html>
            ...
            <body>
                <div id=\"wrapper\">
                    <h1>发生如下错误:</h1>
						{$details}
					<div id=\"copyright\">Powered by <a href=\"http:/" . "/http://www.cloud-biz.cn/\" target=\"_blank\" title=\"{SYS_NAME}官方网站\" style=\"color: #666\"><b>TTTuangou</b> " . defined ( 'SYS_VERSION' ) ? SYS_VERSION : "" . "</a> &nbsp;&copy; 2005 - 2010 <a href=\"http://www.cloud-biz.cn\"  title=\"{SYS_NAME}用户支持\" target=\"_blank\" style=\"color: #666\"><b> Hu La Wang.</b></a></div>
                </div>
            </body>
		</html>";
		return $errer_msg;
