...
class MemberHandler {
	...
	function UpdateSessions() {
		$onlinehold = 900;
		$onlinespan = $this->Config ['onlinespan'] = 15;
		$pvfrequence = 60;
	...
	function SaveActionToLog($title, $auto = true) {
		...
		if ($this->Active ['item_id'] == 0 and @substr_count ( $this->CurrentAction ['action'], $this->Active ['action'] ) > 1) {
			Return '';
		}
	...
	function CheckMember($user, $password, $clsF = true,$kuaisudenglu = false) {
		$this->SetMember ( $user );
		
		if (check_email ( $user )) {
			$type = 'email';
// 			$password_hash = $this->MakePasswordHash ( $password . '@4!@#$%@' );
...
// 			$password_hash = $this->MakePasswordHash ( $password );
		...
		if (trim ( $user ) != '') {
			...
			if ($this->MemberFields ['email2'] == 'zuitu') {
				$password_hash = $this->MakePasswordHash ( $password . '@4!@#$%@' );
			} else {
				$password_hash = $this->MakePasswordHash ( $password );
			}
