...
function referer($default = '?') {
	...
	if ($referer == "" || strpos ( $referer, 'code=register' ) !== false || strpos ( $referer, 'mod=myaccount' ) !== false || (strpos ( $referer, ":/" . "/" ) !== false && strpos ( $referer, $DOMAIN ) === false)) {
...
function page($total_record, $per_page_num, $url = '', $_config = array(), $per_page_nums = "") {
	...
	if ($per_page_num < 1)
		$per_page_num = 10;
	$config ['total_page'] = isset ( $_config ['total_page'] ) ? ( int ) $_config ['total_page'] : 0;
...
function upload_image($image_path = '', $filed = 'face', $small_width = 80, $small_height = 80) {
	...
	if (empty ( $_FILES ) or empty ( $filed )) {
		return array (
				'error' => __ ( '上传图片出错！请检查您的服务器环境！' ) 
		);
	}
	if ($_FILES [$filed] ['name'] == '') {
		return array (
				'error' => __ ( '您没有选择需要上传的图片！' ) 
		);
	}
	$default_type = array (
			'jpg',
	...
	$imgary = explode ( '.', $_FILES [$filed] ['name'] );
	if (! in_array ( strtolower ( $imgary [count ( $imgary ) - 1] ), $default_type )) {
		return array (
				'error' => '不支持的图片格式 [' . $imgary [count ( $imgary ) - 1] . '] ！' 
		);
	}
	...
	$upload_handler->setMaxSize ( 1024 );
	$name = $date . '/' . substr ( md5 ( microtime () ) . '.' . strtolower ( end ( explode ( '.', $_FILES [$filed] ['name'] ) ) ), - 15 );
	...
	if (false == $result) {
		return array (
				'error' => $upload_handler->getError () 
		);
	}
	
	$result = resize_image ( $image_path . $name, $image_path . SMALL_PIC_PREFIX . $name, $small_width, $small_height, false );
	return $name;
}
function image($image, $type = 'face', $is_small = true) {
	...
	if ($is_small == true) {
		if (is_file ( $small_image_file ) !== false)
			return $small_image_file;
		if (is_file ( $image_file ) !== false)
			return $image_file;
	} else {
		if (is_file ( $image_file ) !== false)
			return $image_file;
...
function get_system_env() {
	...
	if (defined ( 'SYS_VERSION' ))
		$e ['sys_version'] = SYS_VERSION;
	if (defined ( 'SYS_BUILD' ))
		$e ['sys_build'] = SYS_BUILD;
	$sys_conf = ConfigHandler::get ();
	if ($sys_conf ['site_name'])
		$e ['sys_name'] = $sys_conf ['site_name'];
	if ($sys_conf ['site_admin_email'])
		$e ['sys_email'] = $sys_conf ['site_admin_email'];
	if ($sys_conf ['site_url'])
		$e ['sys_url'] = $sys_conf ['site_url'];
	if ($sys_conf ['charset'])
		$e ['sys_charset'] = $sys_conf ['charset'];
	if ($sys_conf ['language'])
		$e ['sys_language'] = $sys_conf ['language'];
...
function get_onlineip() {
	...
	preg_match('/\[(.*)\]/', $contents, $m);
	return $m[1];
}

function face_path($uid) {
	$key = "http://www.cloud-biz.cn/";
...
//20141010新增二维码
function qrcode($ticketid,$is_iweb=0){
	$php_path = dirname(__FILE__) . '/';
	//文件保存目录路径
	$save_path = $php_path . '../../';
	$save_path = realpath($save_path) . '/';
	$file_path='images/qrcode/'.date('Y-m-d',time()).'/';
	$ticket_one = logic ( 'coupon' )->SrcOne ( $ticketid );
	if(! empty($ticket_one['path']))
	{
		if (file_exists($save_path.$ticket_one['path'])) {
			if($is_iweb==1){//手机网页
				return $ticket_one['path'];
			}
			echo '<a href='.$ticket_one['path'].'><img src='.$ticket_one['path'].' /></a>';
			return;
		}
	}
	include_once (FUNCTION_PATH . 'qrcode.func.php');

	$img_path=$file_path.$ticket_one['orderid'].$ticket_one['number'].".png";
	$save_path .= $file_path;
	if (!file_exists($save_path)) {
		mkdir($save_path,0777,true);
	}
	$content=$ticket_one['number']."|".$ticket_one['password'];//编号加密码
	QRcode::png($content, $img_path, 'L',4, 2);

	$sql = 'UPDATE ' . table('ticket') . ' SET path ="'.$img_path.'" WHERE ticketid=' . $ticketid;
	dbc()->Query($sql);
	if($is_iweb==1){//手机网页
		return $img_path;
	}
	echo '<a href='.$img_path.'><img src='.$img_path.' /></a>';
}

/**
 * 日志记录
 * @param unknown $log_content 日志内容
 * @param string $log_filename 日志文件名
 */
function logger($log_content,$log_filename="log.txt"){
	$max_size = 10000;
...
function ArrayImgsTransform($arr,$colname='pic',$size = IMG_Original){
...
function ArrayTimeToString(&$arr,$colname,$strFormat='Y-m-d H:i:s')
{
    if(is_array($arr))
        foreach($arr as $key => $p)
        {
            if(is_array($colname))
            {
                foreach($colname as $index => $col)
                {
                    $format = $strFormat;
                    if(is_array($strFormat))$format = $strFormat[$index];
                    $p[$col] =date($format, $p[$col]);
                }
            }
            else
            {
                $p[$colname] =date($strFormat, $p[$colname]);
            }
            $arr[$key] = $p;
        }
    return $arr;
...
function encrypt($str){
	$key = authcode ( $str, 'ENCODE', ini ( 'settings.auth_key' ) );
	//处理url的+问题
	$key = str_replace('+','-',$key);
...
function decrypt($str){
	...
	$strDec = authcode ( $key, 'DECODE', ini ( 'settings.auth_key' ) );
	if ($strDec == '') {
		return authcode ( $str, 'DECODE', ini ( 'settings.auth_key' ) );
	} else {
		return $strDec;
	}
}
function whereAnd(&$str){
    if($str && strlen($str))$str.= ' and ';
}
function whereOr(&$str){
    if($str && strlen($str))$str.= ' or ';
}
...
function attribution($phone){
	...
	return json_decode(json_encode((array) simplexml_load_string($info)), true);
}