...
function page_moyo($sql = '') {
	...
	for($pi = $pfrom; $pi < $pend; $pi ++) {
		...
		if ($pi != $pn)
			...
			$plist .= '<font class="page_current">' . $pi . '</font> ';
	}
...
function page_moyo_request_uri($flag = 'page') {
	...
	foreach ( $_GET as $k => $v ) {
		if ($k != $flag)
			$u .= thtmlspecialchars ( $k ) . '=' . urlencode ( $v ) . '&';
	}
	return substr ( $u, 0, - 1 );
}
...
function imager($id, $size = IMG_Original, $height = 0) {
	...
	if (is_file ( $thumb )) {
		...
		if($size == IMG_Iweb){//Iweb缩略图处理
			$file = logic ( 'image' )->sthumb ( $file ['path'], $thumb, $width, $height );
		}else{
...
function simager($id, $size = IMG_Original, $height = 0) {
    $service_file=ini ( 'settings.qrcode_url' );
    $file404 = ini ( 'settings.site_url' ) . '/static/images/imager.404.jpg';
    ...
    $file = logic ( 'upload' )->GetOne ( $id );
    if (! $file || ($file && ! check_remote_file_exists ( $service_file.$file ['path'] ))) {
        return $file404;
    }
    if ($width == 0 && $height == 0) {
        return $file ['url'];
    ...
    if ($file ['extra'] == '') {
        $extra = handler ( 'image' )->Info ( $service_file.$file ['path'] );
        $data ['extra'] = serialize ( array (
            'width' => $extra ['width'],
            'height' => $extra ['height']
        ) );
    ...
    $srcHeight = $extra ['height'];
    if (abs ( $srcWidth - $width ) * abs ( $srcHeight - $height ) < 1) {
        return $service_file.$file ['url'];
    }
    $upd = UPLOAD_PATH;
    $upt = UPLOAD_PATH . 'thumb/' . $width . 'x' . $height . '/';
    $thumb = str_replace ( $upd, $upt, $file ['path'] );
    if (is_file ( $service_file.$thumb )) {
        $file = $thumb;
    } else {
        $file = logic ( 'image' )->sthumb ( $service_file.$file ['path'],$service_file.$thumb, $width, $height );
    }
...
function msockopen($hostname = false, $port = -1, &$errno = null, &$errstr = null, $timeout = 30) {
...
function admin_priv($priv_str) {
	if (MEMBER_ID > 0 && true === AIJUHE_FOUNDER)
		return true;
	$is_admin_users = in_array ( user ()->get ( 'role_type' ), array (
			'admin',
			'seller' 
	) );
