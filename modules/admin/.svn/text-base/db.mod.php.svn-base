...
class ModuleObject extends MasterObject
{
	...
	function Import()
	{
		...
		if (!$backupdir) {
			...
			foreach($exportlog as $info) {
				...
				($info['version'] != SYS_VERSION ? " onclick=\"return confirm('导入和当前 {SYS_NAME} 版本不一致的数据极有可能产生无法解决的故障，您确定继续吗？');\"" : '').">[导入]</a></td>\n");
			}
		}
	...
	function DoImport()
	{
		...
		if($dumpinfo['method'] == 'multivol') {
			...
			$this->Messager('数据文件非 {SYS_NAME} 格式，无法导入。');
		}
	...
	function DoExport()
	{
		...
		if($method == 'multivol') {
			...
			if(trim($sqldump)) {
				...
				"#\n".
				"# TTTuangou Home: http:\/\/http://www.cloud-biz.cn/\n".
