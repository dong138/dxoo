...
class ModuleObject extends MasterObject {
	...
	function DoAdd() {
		...
		$data ['role_type'] = in_array ( $this->Post ['role_type'], array (
				'normal',
				'admin' 
		) ) ? $this->Post ['role_type'] : 'normal';
		$data ['role_id'] = 0;
		$data ['privs'] = '';
		$data ['regdate'] = time ();
	...
	function DoSearch() {
		...
		extract ( $this->Get );
		$where_list = array ();
		if (in_array ( $ssrc, array (
				'username',
				'phone',
				'email' 
		) ) && $sstr != '') {
			ENC_IS_GBK && $sstr = ENC_U2G ( $sstr );
		...
		$p = max ( $p, 1 );
		$offset = ($p - 1) * $page_num;
		$pages = page ( $total, $page_num, '', array (
				'var' => 'p' 
		) );
		$sql = "
		  SELECT
	...
	function DoModify() {
		...
		$userinfo = dbc ( DBCMax )->query ( 'select uid,username,role_id,role_type,privs,money from ' . table ( 'members' ) . ' where uid=' . $this->Post ['uid'] )->limit ( 1 )->done ();
		if (! $userinfo) {
		...
		$this->Post ['role_type'] = in_array ( $this->Post ['role_type'], array (
				'normal',
				'admin' 
		) ) ? $this->Post ['role_type'] : 'normal';
