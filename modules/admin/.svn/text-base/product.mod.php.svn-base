...
class ModuleObject extends MasterObject {
	...
	function vList() {
		...
		$list = logic ( 'product' )->GetManagerList ( - 1, null, $filter );
	...
	function dList() {
		...
		logic ( 'product' )->AVParser ( $list );
		$drfCount = logic ( 'product' )->GetDraftCount ();
		include handler ( 'template' )->file ( '@admin/dialog_product' );
	}
	function Add() {
		$this->CheckAdminPrivs ( 'product' );
		$p = array ();
	...
	function Add_image() {
		...
		$imgs [] = $id;
		$new = implode ( ',', $imgs );
		logic ( 'product' )->Update ( $pid, array (
				'img' => $new 
		) );
		exit ( 'ok' );
	}
	...
	function Save() {
		...
		// 抽奖的后台处理
		$data ['prize_type'] = post ( 'prize_type', 'txt' );
		$data ['zk1'] = post ( 'zk1', 'txt' );
		$data ['zk2'] = post ( 'zk2', 'txt' );
		$data ['zk3'] = post ( 'zk3', 'txt' );
		$data ['zk1num'] = post ( 'zk1num', 'int' );
		$data ['zk2num'] = post ( 'zk2num', 'int' );
		$data ['zk3num'] = post ( 'zk3num', 'int' );
		...
		$data ['flags'] = post ( 'flags', 'txt' );
		$data ['show_type'] = post ( 'showType', 'int' );
		...
		$data ['is_special'] = post ( 'is_special', 'int' );
		$data ['is_sms'] = post ( 'is_sms', 'int' );
		...
		$data ['daylimit'] = post ( 'daylimit', 'int' );
		$data ['order_time'] = post ( 'order_time', 'int' );
		...
		foreach ( $noNULL as $key => $name ) {
			if ($key == 'nowprice' && is_numeric ( $data [$key] ))
				continue;
			if (! $data [$key]) {
		...
		if ($data ['begintime'] >= $data ['overtime']) {
			$this->Messager ( '团购结束时间不可小于开始时间', - 1 );
		}
		 ...
		 */
		if ($data ['sellerid'] <= 0) {
			$this->Messager ( '请选择合作商家', - 1 );
		}
		if($data ['show_type']!=1 && $data ['show_type']!=2){
			$this->Messager ( '商家显示方式错误', - 1 );
		}
		
		$agentseller = logic ( 'seller' )->GetOne ( $data ['sellerid'] );
		$data ['child_sellerid']= $agentseller['agentid'];
		/**
		 * **验证抽奖折扣及折扣数量****
		 */
		if ($data ['type'] == 'prize') {
			if (($data ['zk1'] == '') || ($data ['zk2'] == '') || ($data ['zk3'] == '')) {
				$this->Messager ( '折扣不可为空', - 1 );
			}
			if ($data ['zk1num'] == '' || $data ['zk1num'] == '0') {
				$this->Messager ( '折扣数量不能小于1', - 1 );
			}
			if ($data ['zk2num'] == '' || $data ['zk2num'] == '0') {
				$this->Messager ( '折扣数量不能小于1', - 1 );
			}
			if ($data ['zk3num'] == '' || $data ['zk3num'] == '0') {
				$this->Messager ( '折扣数量不能小于1', - 1 );
			}
		}
		if (post ( 'imgs' ) != '') {
		...
		$data ['updatetime'] = time ();//20150326 cqx
		$id = post ( 'id', 'int' );
		if ($id == 0) {
			$data ['addtime'] = time ();
		...
		$isDraft && exit ( jsonEncode ( array (
				'status' => 'ok',
				'pid' => $id 
		) ) );
		logic ( 'product' )->ClearDraft ( $id, $draftID );
		$this->Messager ( '产品数据更新完成！', '-2' );
	}
	function Save_intro() {
		$this->CheckAdminPrivs ( 'product', 'ajax' );
		$id = get ( 'id', 'int' );
