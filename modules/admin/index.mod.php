...
class ModuleObject extends MasterObject {
	...
	function main() {
		if (MEMBER_ID < 1) {
			$this->Messager ( "请先在前台进行<a href='index.php?mod=myaccount&code=login'><b>登录</b></a>", null );
		}
		$menuList = $this->Menu ();
	...
	function _recommendList() {
		return array (
				'overtime' => time () + 86400,
				'string' => '<li class="liS"><a href="http:/' . '/www.jishigou.net" target="_blank">建社区站，用记事狗！</a></li><li class="liS"><a href="http:/' . '/tttuangou.net/" target="_blank">地方网站创业：用{SYS_NAME}</a></li><li class="liS"><a href="http:/' . '/biniu.com" target="_blank">比牛，让业务牛逼起来（微信营销平台）</a></li>' 
		);
	}
	function Menu() {
	...
	function home() {
		...
		if (false === ($statistic = cache ( "misc/admin_statistic", 60 ))) {
			...
			foreach ( $item_list as $item => $items ) {
				...
				$sys_env [("sys_" . ("s" == substr ( ($_tmp = str_replace ( array (
						'tttuangou_',
						'system_' 
				), '', $item )), - 1 ) ? $_tmp : $_tmp . "s"))] = $items ['total'];
		...
		$query = $this->DatabaseHandler->Query ( $sql );
		$row = $query->GetRow ();
		$statistic ['cron_length'] = array (
				'name' => '邮件队列长度',
				'url' => 'admin.php?mod=push&code=queue',
				'total' => $row ['total'] 
		);
		$statistic ['data_length'] = array (
				'name' => '数据库尺寸',
				'url' => 'admin.php?mod=db&code=optimize',
				'total' => $data_length 
		);
		$statistic ['express_wait_count'] = array (
				'name' => '等待发货',
				'url' => 'admin.php?mod=delivery&code=vlist&alsend=no',
				'total' => logic ( 'order' )->Count ( 'status=' . ORD_STA_Normal . ' AND process="WAIT_SELLER_SEND_GOODS"' ) 
		);
		...
		$order_nums = dbc ( DBCMax )->select ( 'order' )->in ( 'count(*) as NUM' )->where ( "buytime between {$today_start} and {$today_end}" . $addwhere )->limit ( 1 )->done ();
		$pay_ordes = dbc ( DBCMax )->select ( 'order' )->in ( 'count(*) as NUM,sum(paymoney) as money' )->where ( "paytime between {$today_start} and {$today_end} and paytime > 0 and pay > 0" . $addwhere )->limit ( 1 )->done ();
		$notpay_ordes = dbc ( DBCMax )->select ( 'order' )->in ( 'count(*) as NUM' )->where ( "buytime between {$today_start} and {$today_end} and paytime = 0 and pay = 0" . $addwhere )->limit ( 1 )->done ();
	...
	function recommend() {
		if (false == ($recommend_list = cache ( "misc/recommend_list", 864000 ))) {
			@$recommend_list = request ( "recommend", array (
					'f' => 'text' 
			), $error );
