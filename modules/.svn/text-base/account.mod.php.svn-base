...
class ModuleObject extends MasterObject {
	...
	function Login_done() {
		...
		if ($user && $user ['role_type'] != 'admin' && $user ['checked'] == 0 && ini ( 'product.default_emailcheck' )) {
			header ( 'Location: ' . rewrite ( '?mod=account&code=activate' ) );
			exit ();
		}
	...
	function Login_union() {
		$flag = get ( 'flag', 'txt' );
		if (! $flag || ! ini ( 'alipay.account.login.source.' . $flag ))
			exit ( 'ERROR: no Union Login Request' );
	...
	function Logout() {
		...
		$this->Messager ( $logoutR ['result'] . '退出成功', '/' );
	}
