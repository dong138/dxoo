...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		include($this->TemplateHandler->Template('confirmaccount'));
	}
	function Reset()
	{
		$act_list = array('base'=>'取回密码','reset'=>'重设密码',);
	...
	function choicePassword()
	{
		...
		FROM
			".TABLE_PREFIX. 'system_members'." M LEFT JOIN ".TABLE_PREFIX.'system_memberfields'." MF ON(M.uid=MF.uid)
					WHERE
					BINARY M.email='{$to}' or M.phone='{$to}' or M.username='{$to}'";
		$query = $this->DatabaseHandler->Query($sql);
		$member=$query->GetRow();
		if ($member==false)$this->Messager("用户已经不存在", -1);
...
{$this->iiConfig['site_url']}";
	...
	function PhoneSend()
	{
		...
		if ($member['authstr']!='')
		{
			...
			$inteval=600;			if ($dateline+$inteval>$timestamp)
					{
					$this->Messager("请不要重复恶意发送，如有问题，请与管理员联系。", 0);
			}
		}
		$idstring = random(32);
		$this->DatabaseHandler->SetTable(TABLE_PREFIX.'system_memberfields');
		...
		$onlineip=$_SERVER['REMOTE_ADDR'];
		$action='index.php?mod=get_password&code=reset&uid='.$member['uid'].'&id='.$idstring.'';
		//echo($action);
		include handler ( 'template' )->file ( 'phone_verify_pwd' );
	}
	function DoReset()
	{
		...
		{
			$this->Messager('两次输入的密码不一致,或新密码不能为空。',-1);
		}
		...
		$this->Messager("新密码设置成功,现在为您转入登录界面.","?mod=myaccount&code=login");
	}
