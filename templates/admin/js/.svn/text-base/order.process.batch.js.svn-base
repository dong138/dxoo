...
function doService(obj)
{
    ...
    $.get(rurl, function(data){
        var html = data.replace(/^\s+|\s+$/g, "") == 'ok' ? '<a href="admin.php?mod=order&code=vlist">操作完成，点此返回订单列表</a>' : '<a href="javascript:;" onclick="window.location=window.location;">操作失败，点此刷新</a>';
